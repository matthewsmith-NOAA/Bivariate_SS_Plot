...
par_table <- expand.grid(
  vec1 = seq(75,96,1), #Linf
  vec2 = seq(0.05, 0.15, 0.01) # K
)
...
prof.table <- profile(
  ...
  string = c("L_at_Amax_Fem_GP_1", "VonBert_K_Fem_GP_1"),
...
levelplot(like_matrix, xlab = "Sigma_R", ylab = "Steepness",main="Contour Plot of Steepness and Sigma_R",cuts=50,col.regions=col.l,contour=TRUE,panel=panel.2dsmoother,args=list(span=0.95))
dev.off()

png("Steepness and Sigma_R Contour Plot (Coarse)0.15.png")
col.l <- colorRampPalette(c('blue', 'cyan', 'green', 'yellow', 'orange', 'red'))
levelplot(like_matrix, xlab = "Sigma_R", ylab = "Steepness",main="Contour Plot of Steepness and Sigma_R",cuts=50,col.regions=col.l,contour=TRUE,panel=panel.2dsmoother,args=list(span=0.15))
dev.off()
